...
struct RecordWriter {
  ...
  task<void> operator()(std::pair<CUUID const, CItemData> &p)
  {
...
task<int> PWScore::WriteFile(const StringX &filename, PWSfile::VERSION version,
                       bool bUpdateSig)
{
  ...
  PWSfile *out = co_await PWSfile::MakePWSfile(filename, GetPassKey(), version,
  ...
  try { // exception thrown on write error
    status = co_await out->Open(GetPassKey());
	...
	for (auto it = m_pwlist.begin(); it != m_pwlist.end(); it++)
	{
		co_await write_record(*it);
	}

    // Write attachments (only from V4)
    if (version >= PWSfile::V40)
		for (auto it = m_attlist.begin(); it != m_attlist.end(); it++)
		{
			co_await it->second.Write(out);
		}
	...
	const bool bValidate, const size_t iMAXCHARS)
{
	...
	int closeStatus = co_await in->Close(); // in V3 & later this checks integrity
	delete in;
