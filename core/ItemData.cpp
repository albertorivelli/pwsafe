...
task<size_t> CItemData::WriteIfSet(FieldType ft, PWSfile *out, bool isUTF8) const
{
...
task<int> CItemData::WriteCommon(PWSfile *out) const
{
  int i;
  ...
  for (i = 0; TextFields[i] != END; i++)
    co_await WriteIfSet(TextFields[i], out, true);
  ...
  if (i32 > 0 && i32 <= 3650) {
    putInt(buf32, i32);
    co_await out->WriteField(XTIME_INT, buf32, sizeof(int32));
  }
  ...
  co_await WriteIfSet(PROTECTED, out, false);
  
  co_await WriteUnknowns(out);
  // Assume that if previous write failed, last one will too.
  if (co_await out->WriteField(END, _T("")) > 0) {
    co_return PWSfile::SUCCESS;
  } else {
    co_return PWSfile::FAILURE;
  }
}

task<int> CItemData::Write(PWSfile *out) const
{
  int status = PWSfile::SUCCESS;
  ...
  co_await out->WriteField(UUID, item_uuid, sizeof(uuid_array_t));
  ...
  status = co_await WriteCommon(out);
...
task<int> CItemData::WriteUnknowns(PWSfile *out) const
{
  for (auto uiter = m_URFL.begin();
    ...
    co_await out->WriteField(type, pdata, length);
