...
task<int> PWSfileV3::Open(const StringX &passkey)
{
  //PWS_LOGIT;
...
int PWSfileV3::SanityCheck(FILE *stream)
{
  ...
  // Is file too small?
  const auto file_length = /*pws_os::fileLength(stream)*/0;
...
task<int> PWSfileV3::CheckPasskey(const StringX &filename,
                            const StringX &passkey, IRandomAccessStream^ a_fd,
                            unsigned char *aPtag, uint32 *nITER)
{
  ...
  if (a_fd == nullptr) {
	  a_fd = co_await pws_os::FOpen(filename.c_str(), _T("rb"));
  }
  if (a_fd == nullptr)
    return CANT_OPEN_FILE;
  ...
  a_fd->Seek(sizeof(V3TAG));
  //fseek(fd, sizeof(V3TAG), SEEK_SET); // skip over tag
  DataReader^ dataReader = ref new DataReader(a_fd);
  co_await dataReader->LoadAsync(a_fd->Size);
...
task<size_t> PWSfileV3::ReadCBC(unsigned char &type, unsigned char* &data,
                          size_t &length)
{
  size_t numRead = co_await PWSfile::ReadCBC(type, data, length);
...
task<int> PWSfileV3::ReadHeader()
{
  ...
  TwoFish TF(Ptag, sizeof(Ptag));
  TF.Decrypt(B1B2->Data, m_key);
  TF.Decrypt(B1B2->Data + 16, m_key + 16);
  ...
  TF.Decrypt(B3B4->Data, L);
  TF.Decrypt(B3B4->Data + 16, L + 16);

  m_hmac.Init(L, sizeof(L));

  Array<unsigned char>^ m_ipthin = ref new Array<unsigned char>(TwoFish::BLOCKSIZE);
  dataReader->ReadBytes(m_ipthin);
  dataReader->DetachStream();
  for (int i = 0; i < m_ipthin->Length; i++) m_ipthing[i] = m_ipthin[i];
  ...
  do {
	  auto d = co_await ReadCBC(fieldType, utf8, utf8Len);
	  if (d == 0) continue;
